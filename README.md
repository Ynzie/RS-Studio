...
📺 Watch tutorials and demos on YouTube → @Ynziepoo
---
Features
...
Uses Whisper (via `faster-whisper` / `stable-ts`) to generate word-level timestamps and sync lyrics to audio automatically. RS Studio installs the AI model (~145 MB) on first use — no manual setup needed.
Live Sync & Verify
...
Download from python.org — make sure to tick "Add Python to PATH" during install.
Python packages
Install with pip:
```
...
Custom Song Toolkit installed — provides `packer.exe` and the `Wwise2013.tar.bz2` template
Wwise 2013.2.x installed — provides `WwiseCLI.exe` for audio conversion
...
---
Quick Start
Load a Guitar Pro file — click + Start New Project on the splash screen, or drag a `.gp` file onto the window. RS Studio reads the title, artist, BPM, and track layout automatically.
Fetch audio — paste a YouTube URL into the Audio URL field and click Auto-Fetch. Audio, album art, and synced lyrics are all downloaded automatically.
Check your settings — review Title, Artist, Album, Year, Song Delay, Volume, and which tracks are set to Lead / Rhythm / Bass.
Tune timing — open the Sync & Verify page. Press ▶ Play to hear the audio with notes scrolling over the waveform. Use the Track leadin nudge buttons to shift notes earlier or later. Adjust the Lyrics offset slider if lyrics feel off. Use BPM scale if the GP tempo doesn't match the recording.
Build — click ✓ Build & Export PSARC (or ⚡ Build on the main page). RS Studio converts the audio, generates the chart XMLs, applies dynamic difficulty, and packages the `.psarc`.
Install — copy the `.psarc` into your Rocksmith DLC folder:
```
...
Make sure `yt-dlp.exe` and `ffmpeg.exe` are present in the `tools/` folder next to the app. RS Studio downloads them automatically, but if there was a network error on first launch you can grab them manually from yt-dlp releases and ffmpeg.org.
...
---
Credits
Built on top of the Custom Song Toolkit ecosystem and the incredible work of the Rocksmith modding community.
AI transcription powered by OpenAI Whisper via faster-whisper / stable-ts.
Lyrics sourced from lrclib.net and Genius.
📺 YouTube → @Ynziepoo
